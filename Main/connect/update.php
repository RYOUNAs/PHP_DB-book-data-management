...
$upd_list = $link->query("SELECT * FROM m_book WHERE id =  " . $upd_id . "");
...
if ($title_val == '') {
    ...
    $title_val = $upd_data['title'];
    $volume_val = $upd_data['volume'];
    $price_val = $upd_data['price'];
    $release_date_val = str_replace('-', '', $upd_data['release_date']);
    $purchase_date_val = str_replace('-', '', $upd_data['purchase_date']);
}
...
if ($ok_num >= 5) {
    ...
    if ($ok_purchase == 0) {
        mysqli_query(
            $link,
            "UPDATE m_book SET title='" . $title . "' ,volume=" . $volume . ",price=" . $price . ",release_date='" . $release_date . "',purchase_date=NULL WHERE id=" . $upd_id . ";"
        );
        // echo "UPDATE m_book SET title='" . $title . "' ,volume=" . $volume . ",price=" . $price . ",release_date='" . $release_date . "',purchase_date='NULL' WHERE id='" . $upd_id . "';";
    } else {
        mysqli_query(
            $link,
            "UPDATE m_book SET title='" . $title . "' ,volume=" . $volume . ",price=" . $price . ",release_date='" . $release_date . "',purchase_date='" . $purchase_date_val . "' WHERE id=" . $upd_id . ";"
        );
    }
    ...
    // print_r('</pre>');
    session_start();
    $_SESSION['situation'] = '『' . $title . '』の更新が完了しました';
